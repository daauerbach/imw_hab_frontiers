...
# whole-stream brms: was resto detectable as posterior overlap
...
posterior::summarise_draws(strm_year_meas_mean_fit$fit[[1]])
...
## get posterior draws
## going to need some wrangling due to unique stream/var/col names per complex
## works but will get messy with more than one complex
strm_year_meas_mean_fit_draws <- pmap(list(
  as.list(strm_year_meas_mean_fit$meas),
  as.list(strm_year_meas_mean_fit$cmplx),
  strm_year_meas_mean_fit$fit),
  \(x,y,z){
    as_draws_df(z) |> mutate(meas = x, cmplx = y)
  }) |>
  bind_rows() |>
  select(meas, cmplx, everything())
## also works but also will have unique names per complex...
strm_year_meas_mean_fit_draws <- strm_year_meas_mean_fit |> 
  mutate(drw = map(fit, ~as_draws_df(.x))) |> 
  select(meas, cmplx, drw)
...
# #matrix, no dimnames
# posterior_predict(
#   strm_year_meas_mean_fit$fit[[1]],
#   newdata = strm_year_meas_mean |> 
...
strm_year_meas_mean |> 
  ...
  tidybayes::add_predicted_draws(
    strm_year_meas_mean_fit$fit[[5]]
  ) |> 
  ggplot(aes(x = year, color = cmplx_strm, fill = cmplx_strm)) +
#  tidybayes::stat_lineribbon(aes(y = .prediction), .width = c(.95, .8, .5), alpha = 0.25) +
  tidybayes::stat_lineribbon(aes(y = .prediction), .width = c(.5), alpha = 0.25) +
  geom_point(
    data = strm_year_meas_mean |> 
        filter(cmplx == "ST") |> 
        #select(cmplx, cmplx_strm, year, pct_pools_mean)
        select(cmplx, cmplx_strm, year, vlwd_per100_mean)
    ,
    aes(y = vlwd_per100_mean)
  )
  ...
  mutate(
    pp = map2(
      ...
      ~strm_year_meas_mean |> 
        ...
        ungroup() |> 
        summarise(
          across(.prediction, list(med = ~median(.)))
          , .by = c(cmplx, cmplx_strm, year)
        )  
      )
    ,
    pp = map2(
      data, pp,
      ~inner_join(.x, .y, by = c("cmplx_strm", "year")) |> 
        select(-cmplx)
      )
    )
...
strm_year_meas_mean_pp |> 
  ...
  unpack(c(pp)) |> 
  ggplot(aes(x = year, color = cmplx_strm, fill = cmplx_strm)) +
  geom_line(aes(y = .prediction_med)) +
...
#probably good enough not to bother doing something more custom
strm_year_meas_mean_pp |> 
  mutate(
    gg = map2(
      pp, data,
      ~ggplot(data = .x, aes(x = year, color = cmplx_strm, fill = cmplx_strm)) +
        tidybayes::stat_lineribbon(aes(y = .prediction), .width = c(.5), alpha = 0.25, linewidth = 1.1) +
        geom_point(data = .y, aes(y = val)) +
...
```{r param_differences_NOT_STARTED}
