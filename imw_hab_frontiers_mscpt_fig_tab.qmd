...
#no exact 0 or 1 in either pct_*_mean measure (or 0s in any others)
#so not adjusting by small value
#3 NAs for ST streams in 2017
#reorder stream name factor to have reference first in each complex
#HC and LC already are, but switch deep and w twin
strm_year_meas_mean <- strm_year |> 
  ...
  mutate(
    ...
    cmplx_strm = factor(cmplx_strm, levels = c(
      "HC_Stavis Creek*", "HC_Seabeck Creek", "HC_Big Beef Creek", "HC_Little Anderson Creek",
      "LC_Mill Creek*", "LC_Abernathy Creek", "LC_Germany Creek", 
      "ST_West Twin River*", "ST_Deep Creek", "ST_East Twin River"
    )),
    year_scl = as.numeric(scale(year, center = TRUE, scale = sd(year)))
  ) 

strm_year_meas_mean_fit <- readRDS("strm_year_meas_mean_fit.rds")

#revised to make ref streams dark grey and boost vis contrasts
pal_strm <- set_names(
  c(c("grey20", "#3A270A", "tomato", "#965127"),
    c("grey20", "darkblue", "cyan"),
    c("darkgreen","grey20", "#516F25")
    ),
...
```{r site_low_high_variation_CV}

#at least for bankfull W:D, do not have inflationary decimal values
d <- site_meta |> 
  mutate(
        cmplx_strm = factor(cmplx_strm, levels = c(
  ...
  select(cmplx_strm, site, year_min:year_n, 
         contains("bankfull_W_D") , contains("bankfull_width"), contains("pool_bnk_depth")
         ) |> 
  #arrange(bankfull_W_D_mean)
  #arrange(bankfull_W_D_sd)
  mutate(
    #cv = bankfull_W_D_sd / bankfull_W_D_mean
    bankfull_W_D_cv = bankfull_W_D_sd / bankfull_W_D_mean,
    bankfull_width_cv = bankfull_width_sd / bankfull_width_mean,
    pool_bnk_depth_cv = pool_bnk_depth_sd / pool_bnk_depth_mean
  )

#distributions across sites of per-site CV through time
#just W:D
d |> 
  ggplot() + 
  geom_boxplot(
    aes(cmplx_strm, bankfull_W_D_cv, fill = cmplx_strm, color = cmplx_strm), 
    varwidth = T, alpha = 0.4, show.legend = F) +
  scale_color_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) +
  scale_x_discrete(guide = guide_axis(angle = 270))

#all 3, W:D, bankfull and depth
{
d |> 
  select(cmplx_strm, site, ends_with("_cv")) |> 
  pivot_longer(ends_with("_cv"), names_to = "meas", values_to = "cv") |> 
  mutate(
    meas = case_when(
     meas == "bankfull_W_D_cv" ~ "Bankfull W:D",
     meas == "bankfull_width_cv" ~ "Bankfull width",
     meas == "pool_bnk_depth_cv" ~ "Pool depth"
  )) |> 
  ggplot() + 
  geom_boxplot(
    aes(cmplx_strm, cv, fill = cmplx_strm, color = cmplx_strm), 
    varwidth = T, alpha = 0.4, show.legend = F) +
  scale_color_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) +
  scale_x_discrete(guide = guide_axis(angle = 270)) +
  facet_wrap(~meas, ncol = 1) + #scales = "free_y"
  labs(x = NULL, y = "CV")
}+{
d |> 
  mutate(cmplx = str_sub(cmplx_strm, 1,2)) |> 
  ggplot() +
  geom_point(aes(bankfull_width_mean, bankfull_W_D_cv, color = cmplx_strm), show.legend = F) +
  scale_color_manual(values = pal_cmplx_strm) +
  facet_wrap(~cmplx) +
    labs(
      x = "Per-site bankfull width mean over time",
      y = "Per-site W:D coef. var. over time"
    )
}

ggsave(filename = "rev_fig_site_cv_WD_box_points.png", width = 11, height = 9, dpi = 300, bg = "white")


#measures through time for hi/lo CV
#different way to capture that difference between hi and low CV is not actually that big
bind_rows( 
  semi_join(site_year, d |> slice_max(bankfull_W_D_cv, n = 5, by = cmplx_strm), by = "site") |> mutate(type = "max-CV"),
  semi_join(site_year, d |> slice_min(bankfull_W_D_cv, n = 5, by = cmplx_strm), by = "site") |> mutate(type = "min-CV")
  ) |> 
  mutate(type = factor(type, levels = c("min-CV","max-CV"))) |> 
  ggplot(aes(year, bankfull_W_D, group = site, color = cmplx_strm)) +
  scale_color_manual(values = pal_cmplx_strm) +
  geom_line(linewidth = 0.3, show.legend = F) + geom_point(size = 0.5, show.legend = F) +
  facet_grid(rows = vars(cmplx_strm), cols = vars(type), scales = "free") +
  labs(
    title = "Sites per stream with smallest and largest interannual CV in bankfull W:D"
  ) +
  theme_light()
...
#whole stream...spatial CV and grand CV

##Evolution in time of SPATIAL per-year whole-stream CV: rebuild since took SD and CV off
site_year |>
  summarise(
    n_site_with_bnk = length(na.omit(bankfull_width)),
    across(
  ...
  #arrange(bankfull_W_D_cv)
  arrange(desc(bankfull_W_D_cv)) |> 
  ggplot(aes(year, bankfull_W_D_cv, color = cmplx_strm)) +
  geom_line() +
  scale_color_manual(values = pal_cmplx_strm, aesthetics = c("color","fill"))

#through time "grand" CV of whole-stream mean
#lower ("stabilized") than box
strm_year |> 
  select(cmplx_strm, year, contains("bankfull_W_D")) |> 
  summarise(
    across(
      bankfull_W_D_mean,
      list(
    ...
    .by = cmplx_strm) |> 
    mutate(bankfull_W_D_cv = bankfull_W_D_mean_sd / bankfull_W_D_mean_mean)
...
per WD: STA026 is mislabeled, It should be BIB026
not yet corrected 12/8/25
...
# was resto detectable: whole-stream brms of measure ~ year | stream

```{r testing_brms_marss}
#modified from chatgpt https://chatgpt.com/c/693728cf-7c24-832d-8417-34fb4da206f5

# install.packages("brms")   # or use cmdstanr + brms backend if preferred
library(brms)

# Data expectations:
# df should contain columns: stream (factor), year (numeric; actual year or 1:18),
# pct_g (numeric between 0 and 1, no exact 0/1; see note below),
# treatment (factor with levels c("ref","trt1","trt2")), complex (if needed)

# NOTE: attempting to use stream name string only since 'treatment' is the stream 
# scale year (helps sampling) and 
# If you have any exact 0 or 1 values in pct_g, transform to keep within (0,1):
eps <- 1e-4

df <- strm_year |> #summary()
  #mutate(trtmt = if_else(str_detect(cmplx_strm, "West|Stavis|Mill"), paste0(cmplx_strm, "_ref"), paste0(cmplx_strm, "_trt")))
  filter(str_detect(cmplx_strm, "LC")) |> 
  #select(cmplx_strm, year, pct_g = pct_gravel_mean) |> 
  #select(cmplx_strm, year, pct_g = pct_pools_mean) |> 
  select(cmplx_strm, year, vlwd = vlwd_per100_mean) |> 
  mutate(
    cmplx_strm = factor(cmplx_strm), #may want to be explicit about levels of inline refactoring
    year_s = as.numeric(scale(year, center = TRUE, scale = sd(year)))
    # ,
    # pct_g_adj = pmin(pmax(pct_g, eps), 1 - eps)
    )

df |> summary()

# regression with stream-level intercepts & slopes, AR1 within stream
formula_pctg <- brms::bf(
  #pct_g_adj ~ 1 + cmplx_strm * year_s +   # main & interaction (treatment-specific slopes)
  vlwd ~ 1 + cmplx_strm * year_s +   # main & interaction (treatment-specific slopes)
              (1 + year_s | cmplx_strm)   # random intercept + slope per stream
  ,
  # AR(1) per stream across years
  autocor = ~ brms::ar(time = year, gr = cmplx_strm, p = 1)
  )

# 3) Priors (regularizing)
priors <- c(
  prior(normal(0, 2), class = "Intercept"), # on logit scale
  prior(normal(0, 0.8), class = "b"),       # fixed effects
  prior(student_t(3, 0, 1), class = "sd"),  # random effect SDs
  prior(lkj(2), class = "cor") #,             # correlation matrix prior for random effects
  #prior(gamma(0.33, 0.33), class = "phi")   # phi: Beta precision (orig suggested 'informative-ish' throw all sorts of errors)
)

# 4) Fit the model (robust defaults for small sample; adapt chains/iter as needed)
fit_pctg <- brm(
  formula = formula_pctg,
  data = df,
...
# 5) Quick diagnostics & results
summary(fit_pctg)
pp_check(fit_pctg, ndraws = 100)
#names(as_draws_df(fit_pctg))
plot(bayesplot::mcmc_trace(as_draws_df(fit_pctg), pars = c("phi")))
plot(bayesplot::mcmc_trace(as_draws_df(fit_pctg), pars = vars(starts_with("b_")))) 

# 6) Extract posterior contrasts: slope differences (treatment vs ref)
# In model, treatment × year_s coefficients give treatment-specific slope offsets
# Example: compute posterior of difference in slope between trt1 and ref

post <- as_draws_df(fit_pctg) #posterior_samples(fit_pctg)

# Identify parameter names:
# "b_year_s" is baseline slope (for reference level of treatment)
# "b_treatmenttrt1:year_s" is offset for trt1, etc.
# So slope_trt1 = b_year_s + b_treatmenttrt1:year_s
# then calc difference of interaction term posteriors
post |> 
  mutate(
    slope_ref = `b_year_s`,
    slope_abe = `b_year_s` + `b_cmplx_strmLC_AbernathyCreek:year_s`,
    slope_ger = `b_year_s` + `b_cmplx_strmLC_GermanyCreek:year_s`,
    diff_abe_ref = slope_abe - slope_ref,
    diff_ger_ref = slope_ger - slope_ref
  ) |> 
  select(
    #starts_with("slope"), 
    starts_with("diff")) |> 
  summarise(
    diff_abe_ref_mean = mean(diff_abe_ref),
    diff_abe_ref_pgt0 = mean(diff_abe_ref > 0),
    diff_ger_ref_mean = mean(diff_ger_ref),
    diff_ger_ref_pgt0 = mean(diff_ger_ref > 0)
  )

# 7) Posterior predicted trajectories (example): compute fitted trajectories for each cmplx_strm
# Create a newdata grid
newdata <- expand.grid(
  year = seq(min(df$year), max(df$year), length.out = 18),
  #treatment = levels(df$treatment),
  cmplx_strm = levels(df$cmplx_strm)
  ) |> 
  mutate(
    year_s = as.numeric((year - mean(df$year))/sd(df$year))
    # ,
    # pct_g_adj = NA
  )

# fitted_post is S x N matrix: summarise median and 95% CI for plotting
fitted_post <- posterior_epred(fit_pctg, newdata = newdata, re_formula = NULL, allow_new_levels = TRUE)

#likely a tidyier way to do this to ensure N colnames match the newdata expanded grid levels
# #same thing but uglier than base-r version suggested...
# summarise(as_tibble(fitted_post), across(everything(), list(med = ~median(.), lwr = ~quantile(., 0.025), upr = ~quantile(., 0.975)))) |> 
#   pivot_longer(everything()) |> 
#   mutate(
#     var = str_sub(name, -3, -1), 
#     name = str_remove_all(name, "med|lwr|upr")
#     ) |> 
#   pivot_wider(names_from = "var", values_from = "value")
  
fdf <- newdata |> 
  bind_cols(
    as.data.frame(t(apply(fitted_post, 2, function(x) c(median = median(x), lwr = quantile(x, 0.025), upr = quantile(x, 0.975)))))    
  ) |> 
  left_join(
    df |> select(-year_s), by = c("cmplx_strm", "year"), suffix = c("_fit","_obs")
  )

fdf |> 
  ggplot(aes(x = year, color = cmplx_strm, fill = cmplx_strm)) +
  #scale_color_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) +
  geom_ribbon(aes(ymin = `lwr.2.5%`, ymax = `upr.97.5%`), alpha = 0.2) +
  geom_line(aes(y = median), linetype = 1, linewidth = 1.1) +
  #geom_point(aes(y = pct_g_adj_obs)) + geom_line(aes(y = pct_g_adj_obs), linetype = 3) +
  geom_point(aes(y = vlwd)) + geom_line(aes(y = vlwd), linetype = 3) +
  labs(
    #title = "Test brms fit of percent gravel on year in LC, obs as points+dashed",
    title = "Test brms fit of vlwd100 on year in LC, obs as points+dashed",
    subtitle = deparse(formula_pctg$formula)
  )


## alternatively, via https://nwfsc-timeseries.github.io/MARSS-Manual/ar1-observed-with-error.html
## https://nwfsc-timeseries.github.io/MARSS-Manual/trend-observed-with-ar1-error.html
library(MARSS)

#https://nwfsc-timeseries.github.io/MARSS-Manual/rw-observed-with-multiple-time-series.html
#single process, 3 obs series
Z <- matrix(1, 3, 1)
R <- "diagonal and equal"
#data need to be wide: row sites by col years
#yt <- t(harborSealWA)[c("EBays", "PSnd", "HC"), ]
yt <- df |> select(cmplx_strm, year, pct_g_adj) |> pivot_wider(names_from = year, values_from = pct_g_adj)
rn <- yt$cmplx_strm
yt <- as.matrix(yt[,-1])
rownames(yt) <- rn
fit1 <- MARSS(yt, model = list(R = R, Z = Z)) #works
MARSS(yt, model = list(R = "diagonal and equal", Z = Z)) #works
MARSS(yt, model = list(R = "diagonal and equal", Z = "identity")) #does not
broom::tidy(fit1)
plot(fit1, plot.type = "fitted.ytT")

#3 processes, correlated process error (throwing error, looks like eqn does not match objects and 'fit' call in page)
##https://nwfsc-timeseries.github.io/MARSS-Manual/multiple-time-series-each-observing-a-rw.html
Q <- "unconstrained"
R <- "diagonal and equal"
#all of these error
fit2 <- MARSS(yt, model = list(R = R, Q = Q))
fit2 <- MARSS(yt, model = list(R = "diagonal and equal", Q = "unconstrained", Z = "identity", ))
fit2 <- MARSS(yt, model = list(Z = diag(1,3,3), R = R, Q = Q))
fit2 <- MARSS(yt, model = list(Z = "identity", R = R, Q = Q))
fit2 <- MARSS(yt, model = list(Z = Z, U = "unequal", R = R, Q = Q))
...
```{r fitting}
#tibble of measure-complex data subsets
strm_year_meas_mean_nest <- strm_year_meas_mean |> 
  pivot_longer(cols = -c(cmplx, cmplx_strm, year, year_scl), names_to = "meas", values_to = "val") |> 
  nest(.by = c(meas, cmplx)) |> 
  mutate(fam = if_else(str_sub(meas,1,3)=="pct", "beta","lognorm")) |> 
  arrange(meas, cmplx)

# regression with stream-level intercepts & slopes, AR1 within stream
strm_year_formula <- brms::bf(
  # #orig combined FE and RE: 
  # val ~ 1 + cmplx_strm * year_scl + (1 + year_scl | cmplx_strm)   
  # examined FE of stream on year with RE intercepts by stream but added little, and very slightly worse loocv
  # val ~ 1 + cmplx_strm * year_scl + (1 | cmplx_strm)
  # so reducing to simplest and clearest (and fastest) FE-only
  val ~ 1 + cmplx_strm * year_scl 
  ,
  autocor = ~ brms::ar(time = year_scl, gr = cmplx_strm, p = 1)
  )

#strm_year_meas_mean_fit_orig <- strm_year_meas_mean_fit

#iterate fitting per measure and complex, switching priors and family as needed
#chains run fast, stan compilation takes as long as reasonable iterations
strm_year_meas_mean_fit <- strm_year_meas_mean_nest |> 
  mutate(
    fit = map2(
      data, fam,
      \(x,y){
        #base priors for lognorm, add extra for phi if beta fam
        fmly = brms::lognormal() #default link is identity, expects original not log scale values
        prs = c(
          prior(normal(0, 2), class = "Intercept"), # on logit scale
          prior(normal(0, 0.8), class = "b")       # fixed effects
          #,prior(student_t(3, 0, 1), class = "sd")  # random effect SDs
        ...
        if(y == "beta"){
          fmly = brms::Beta(link = "logit")
          prs = c(prs, prior(gamma(0.33, 0.33), class = "phi"))
        }
        
        f = brm(
          formula = strm_year_formula,
          data = x,
          family = fmly,
          prior = prs,
...
saveRDS(strm_year_meas_mean_fit, "strm_year_meas_mean_fit.rds")

```

[Methods]

With the focus in this paper on whether and how treatment stream observations diverged detectably from those in reference streams, we regressed the whole stream annual habitat measures against year, fitting Bayesian multilevel models per complex of each response measure against the stream-year interaction, allowing intercept and slope effects per stream and with AR(1) autocorrelation in the year (R version 4.5.1, R Core Team 2025; package `brms` 2.23.0, Bürkner 2017). This straightforward analysis approach was adopted following extensive exploration of the data with a variety of visualization and statistical options. The proportion gravel and proportion pools were specified with a beta response family, a lognormal was used for the remaining (positive, real) measures, and all models used relatively uninformative priors. Model diagnostics showed good convergence with 3000 iterations on 4 chains. In addition to examining posterior predictions and parameter estimates, we calculated pairwise Bhattacharyya coefficients for each treatment stream parameter distribution relative to the reference as a means of evaluating the overlap in the estimates of year effects (i.e., evaluating the consistency of both central tendency and variation in slopes through time), with 1 meaning perfect agreement and 0 indicating no overlap (package `dispRity`, Guillerme 2018).

[Results]

Models of whole-stream habitat measures indicated little difference among streams in the magnitude and direction of consistent change through time. In most cases, differences among streams within each complex that were apparent in 2007 persisted throughout data collection, showing few signs of the effects of interventions (Figure NNN posterior preds) [*showing spread in the y-axis values among streams is similar at the beginning, middle, and end of the series despite any up/down-slope*]. Increases or decreases in many measures were weakly discernible and quite uncertain, with posterior distributions and both 95% and 50% credible intervals for the per-stream effects of year centered on and including 0 (in log link space). Most of these distributions were also highly overlapping among streams (Figures: param distributions), with Bhattacharyya coefficient values near 1 (table Bhatt coef). 

Posterior predictions for pool frequency and depth measures offered no strong evidence that treatment stream trajectories differed from those in the Mill, Stavis and West Twin reference series. Big Beef Creek was the only system to exhibit even modest separation from other streams in terms of an increase in pool frequency and depth. Consistent with the common field observation of bedload deposition and aggradation, the estimated change in whole-stream pool depth was negative (shallower) in all streams with the exception of a relatively uncertain and weakly positive trend in Germany Creek. Moreover, the slight departures found in the Deep Creek and Little Anderson series were towards a greater decrease in depth (figs panels D, E, F). 

The whole-stream average bankfull width to bankfull depth ratios showed evidence of increases through time in all streams but Big Beef, consistent with the indications of slighly decreasing average pool depths. The estimated increase in this measure in Germany Creek was appreciably greater than in Abernathy or Mill (Bhatt. coef. 0.64 vs Mill), and the estimates for Little Anderson and Big Beef were distinct from that for Stavis (and Seabeck) Creek, though with opposing directions (greater increase in Little Anderson; no change in Big Beef). The predicted proportion of gravel substrate followed similar trajectories in the Straits and Lower Columbia streams, with no substantive differences in the estimated rates of increase for the Straits and Germany Creek having a larger but still overlapping increase. Estimates for all streams in Hood Canal included an appreciable probability of no change, but Seabeck Creek had a median decrease while the others were slightly increasing. 

The clearest evidence of restoration interventions at the whole-stream scale was apparent in the measures of very large wood in Abernathy Creek. The series of both piece count per 100m and volume showed plain departures from the Mill Creek reference (and Germany Creek), with no overlap in 50% credible intervals and the lowest Bhattacharyya coefficient values in the analysis. The increase shifted the estimated instream wood in Abernathy the least at the outset of data collection in 2007 to the most in 2024, with the difference strengthening after approximately 2019. However, tn the Straits and Hood Canal, despite small differences among streams in the median change through time, the overall posterior distributions of estimate were highly overlapping. A weakling increasing trajectory in Little Anderson Creek differed slightly from the weak decrease in the reference Stavis Creek.
...
Guillerme T., 2018. dispRity: A modular R package for measuring disparity. Methods Ecol Evol. 2018;00:1-9. https://doi.org/10.1111/2041-210X.13022

```{r}
lm_eq <- lm(val ~ 1 + cmplx_strm * year_scl, data = strm_year_meas_mean_nest$data[[1]]) 
equatiomatic::extract_eq(lm_eq)
equatiomatic::eq_(lm_eq)
```

$$ \operatorname{measure} = \alpha_{ref-stream} + \beta_{trtmnt-stream_i} + \beta_{ref-stream}(\operatorname{year}) + \beta_{trtmnt-stream_i}(\operatorname{stream} \times \operatorname{year}) + \epsilon $$

```{r draws_summary}
# summary(strm_year_meas_mean_fit$fit[[1]])
# #same but better tibble format
# as_draws_df(strm_year_meas_mean_fit$fit[[1]]) |> posterior::summarise_draws()
# #can also work directly on fit object but want draws for other stuff
# posterior::summarise_draws(strm_year_meas_mean_fit$fit[[1]]) |> print(n = Inf)
# 
# plot(strm_year_meas_mean_fit$fit[[1]], nvariables = 10)
# conditional_effects(strm_year_meas_mean_fit$fit[[1]])
# conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "cmplx_strm")
# conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl")
# conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm") 
# #not immediately obvious a quicker way to manipulate this ggplot object to unscale the year axis... 
# str( plot(conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm"), plot = F)[[1]] )

# #default version with smooth continuous scaled year
# map2(
#   strm_year_meas_mean_fit$fit,
#   strm_year_meas_mean_fit$meas,
#   ~plot(conditional_effects(.x, effects = "year_scl:cmplx_strm"), plot = F)[[1]] +
#     labs(subtitle = .y)
# )


## single tibble of main summary info
fit_summary <- pmap(list(
  as.list(strm_year_meas_mean_fit$meas),
  as.list(strm_year_meas_mean_fit$cmplx),
  strm_year_meas_mean_fit$fit),
  \(x,y,z){
    posterior::summarise_draws(z) |>
      mutate(meas = x, cmplx = y)
  }) |>
  bind_rows() |> 
  mutate(
    meas = case_when(
      meas == "bankfull_W_D_mean" ~ "Bankfull W:D",
      meas == "pct_gravel_mean" ~ "Ppn. gravel",
      meas == "pct_pools_mean" ~ "Ppn. pool",
      meas == "pool_bnk_depth_mean" ~ "Pool depth",
      meas == "vlwd_per100_mean" ~ "VLWD/100m",
      meas == "vlwd_vol_mean" ~ "VLWD vol."
    )
    ,
    cmplx_strm = str_remove_all(variable, "b_cmplx_strm|:year_scl"),
    cmplx_strm = case_when(
      cmplx_strm == "b_year_scl" & cmplx == "HC" ~ "HC_Stavis Creek*",
      str_detect(cmplx_strm, "Seabeck") ~ "HC_Seabeck Creek",
      str_detect(cmplx_strm, "Beef") ~ "HC_Big Beef Creek",
      str_detect(cmplx_strm, "Anderson") ~ "HC_Little Anderson Creek",
      cmplx_strm == "b_year_scl" & cmplx == "LC" ~ "LC_Mill Creek*",
      str_detect(cmplx_strm, "Abernathy") ~ "LC_Abernathy Creek",
      str_detect(cmplx_strm, "Germany") ~ "LC_Germany Creek",
      cmplx_strm == "b_year_scl" & cmplx == "ST" ~ "ST_West Twin River*",
      str_detect(cmplx_strm, "Deep") ~ "ST_Deep Creek",
      str_detect(cmplx_strm, "East") ~ "ST_East Twin River"
    ),
    cmplx_strm = factor(cmplx_strm, levels = levels(strm_year_meas_mean$cmplx_strm))
  ) |> 
  select(meas, cmplx, everything())

#could table-ize but need to clear that slope est values are offsets from ref
fit_summary |> 
  filter(str_detect(variable, "year_scl")) |> 
#  filter(str_detect(variable, "cor|r_cmplx|sd", negate = T)) |> 

  select(meas, cmplx, cmplx_strm, mean, q5, q95, rhat, ess_bulk) |> 
  gt(groupname_col = "meas") |> 
  fmt_number(c("mean", "q5", "q95"), decimals = 4) |>  
  fmt_number("rhat", decimals = 2) |> 
  fmt_number("ess_bulk", decimals = 0) 


```

```{r posterior_pred_series}
#tidybayes::add_predicted_draws for consistent well-named tibble format
#leaving defaults of re_formula=NULL for all group level effects
#can summarise to desired quantiles inline, but 
#tidybayes::stat_lineribbon does it already with '.width' argument...
#already have 'data' col on the _fit tibble so can directly access for plotting without joining
strm_year_meas_mean_pp <- strm_year_meas_mean_fit |> 
  mutate(
    meas_gg = case_when(
      meas == "bankfull_W_D_mean" ~ "Bankfull W:D",
      meas == "pct_gravel_mean" ~ "Ppn. gravel",
    ...
    pp = map2(
      ...
      ~strm_year_meas_mean |> 
        filter(cmplx == .x) |> 
        select(cmplx, cmplx_strm, year, year_scl) |> 
        mutate(cmplx_strm = droplevels(cmplx_strm)) |> 
        tidybayes::add_predicted_draws(.y)
      )
    ,
    gg = map2(
      pp, data,
      ~ggplot(data = .x, aes(x = year, color = cmplx_strm, fill = cmplx_strm)) +
        tidybayes::stat_lineribbon(aes(y = .prediction), .width = c(0.5), alpha = 0.25) +
        geom_line(data = .x |> summarise(pred_med = median(.prediction), .groups = "drop"),
                  aes(y = pred_med)) +
        geom_point(data = .y, aes(y = val), size = 0.6) +
        geom_line(data = .y, aes(y = val), linetype = 3)
      )
    ,
    gg = map2(
      gg, meas_gg, ~.x + 
        ...
        labs(
          #subtitle = .y,
          x = NULL, y = .y, color = NULL, fill = NULL)
      )
    )
...
#pools
strm_year_meas_mean_pp |> 
  filter(str_detect(meas, "pool")) |> 
  pull(gg) |> 
  wrap_plots(nrow = 3, byrow = F, guides = "collect") +
  plot_annotation(tag_levels = "A")

ggsave("rev_fig_pp_series_pools.png", height = 14, width = 11, bg = "white", dpi = 150)

#wood
strm_year_meas_mean_pp |> 
  filter(str_detect(meas, "vlwd")) |> 
  pull(gg) |> 
  wrap_plots(nrow = 3, byrow = F, guides = "collect") +
  plot_annotation(tag_levels = "A")

ggsave("rev_fig_pp_series_wood.png", height = 14, width = 11, bg = "white", dpi = 150)

#W:D and gravel
strm_year_meas_mean_pp |> 
  filter(str_detect(meas, "bankfull|gravel")) |> 
  pull(gg) |> 
  wrap_plots(nrow = 3, byrow = F, guides = "collect") +
  plot_annotation(tag_levels = "A")

ggsave("rev_fig_pp_series_WDgrvl.png", height = 14, width = 11, bg = "white", dpi = 150)

```

```{r posterior_params}
# #tidybayes::get_variables(strm_year_meas_mean_fit$fit[[1]]) 
# #underlying posterior:: version
# as_draws_df(strm_year_meas_mean_fit$fit[[1]], variable = "^b_.*year_scl", regex = T) |> 
#   unpack(c(.chain, .iteration, .draw))
# #with indexing cols shifted
# tidybayes::spread_draws(strm_year_meas_mean_fit$fit[[1]], `^b_.*year_scl$`, regex = T) |> 
#   mutate(across(contains(":"), ~b_year_scl + .))


#make another object with list cols of param posterior draws
#first generate coef estimate for stream*year interaction as reference+treatment offset 
#then list of Bhatt coef per complex & measure
#then list of ggplots (coercing stream factor levels back to palette names)
strm_year_meas_mean_drw_sprd <- strm_year_meas_mean_fit |> 
  mutate(
    meas = case_when(
      meas == "bankfull_W_D_mean" ~ "Bankfull W:D",
      meas == "pct_gravel_mean" ~ "Ppn. gravel",
    ...
    drw = map(
      fit,
      ~tidybayes::spread_draws(.x, `^b_.*year_scl$`, regex = T) |> 
        mutate(across(contains(":"), ~ b_year_scl + .)) #sum the ref and trt interaction offset 
      ),
    bc = map(
      drw,
      ~.x |> 
        summarise(
          across(
            contains(":"),
            list(bc = ~dispRity::bhatt.coeff(b_year_scl, .))
          )
        ) |> 
        pivot_longer(ends_with("bc"), names_to = "Stream", values_to = "val")
    ),
    gg = map2(
      drw, cmplx,
      ~.x |> 
        pivot_longer(starts_with("b_"), names_to = ".variable", values_to = ".value") |> 
        mutate(
          cmplx_strm = str_remove_all(.variable, "b_cmplx_strm|:year_scl"),
          cmplx_strm = case_when(
            cmplx_strm == "b_year_scl" & .y == "HC" ~ "HC_Stavis Creek*",
            str_detect(cmplx_strm, "Seabeck") ~ "HC_Seabeck Creek",
            str_detect(cmplx_strm, "Beef") ~ "HC_Big Beef Creek",
            str_detect(cmplx_strm, "Anderson") ~ "HC_Little Anderson Creek",
            cmplx_strm == "b_year_scl" & .y == "LC" ~ "LC_Mill Creek*",
            str_detect(cmplx_strm, "Abernathy") ~ "LC_Abernathy Creek",
            str_detect(cmplx_strm, "Germany") ~ "LC_Germany Creek",
            cmplx_strm == "b_year_scl" & .y == "ST" ~ "ST_West Twin River*",
            str_detect(cmplx_strm, "Deep") ~ "ST_Deep Creek",
            str_detect(cmplx_strm, "East") ~ "ST_East Twin River"
            ),
          cmplx_strm = factor(cmplx_strm, levels = levels(strm_year_meas_mean$cmplx_strm))
          ) |> 
        ggplot(aes(color = cmplx_strm, fill = cmplx_strm)) +
        scale_color_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) +
        geom_vline(xintercept = 0, linetype = 2, linewidth = 0.5) +
        tidybayes::stat_halfeye(aes( .value, cmplx_strm), alpha = 0.3, show.legend = F)
      ),
    gg = map2(
      gg, meas,
      ~.x + labs(y = NULL, x = expression(beta["Stream*year"]), subtitle = .y)
    )
  ) |> 
  select(meas, cmplx, drw, bc, gg)


## do estimates of year effects (slope) for treatment streams differ from reference stream? and from 0?

#table of Bhattacharyya coefficients (0 strongly different to 1 identical distributions) for treatment streams pairwise vs ref stream per complex
strm_year_meas_mean_drw_sprd |> 
  select(meas, cmplx, bc) |> 
  unnest_longer(bc) |> unpack(bc) |> 
  pivot_wider(names_from = meas, values_from = val) |> 
  mutate(Stream = str_remove_all(Stream, "b_cmplx_strm|:year_scl_bc")) |> 
  #writexl::write_xlsx("tbl_bhatt_coef.xlsx")
  gt(
    groupname_col = "cmplx", 
    caption = md("Bhattacharyya coef. of pairwise overlap<br>in treatment & reference posterior distributions of year effect")
  ) |> 
...
##plot posteriors of slope estimate (year effect), showing overlap of 0 as well as mutual overlap
##again by measure "type" for tractable plots
#pools
strm_year_meas_mean_drw_sprd |> 
  filter(str_detect(meas, "ool")) |> 
  pull(gg) |> 
  wrap_plots(nrow = 3, byrow = F, guides = "collect") +
  plot_annotation(tag_levels = "A")

ggsave("rev_fig_coef_pools.png", height = 9, width = 11, bg = "white", dpi = 150)
...
ggsave("rev_fig_coef_wood.png", height = 9, width = 11, bg = "white", dpi = 150)
...
ggsave("rev_fig_coef_WDgrvl.png", height = 9, width = 11, bg = "white", dpi = 150)



strm_year_meas_mean_fit[[1]] 

summary(strm_year_meas_mean_fit_orig$fit[[1]])
summary(strm_year_meas_mean_fit$fit[[1]])

fixef(strm_year_meas_mean_fit_orig$fit[[1]])
fixef(strm_year_meas_mean_fit$fit[[1]])

ranef(strm_year_meas_mean_fit_orig$fit[[1]])
ranef(strm_year_meas_mean_fit$fit[[1]])

coef(strm_year_meas_mean_fit_orig$fit[[1]])
coef(strm_year_meas_mean_fit$fit[[1]])

loo::loo_compare(
loo::loo(strm_year_meas_mean_fit_orig$fit[[1]]),
loo::loo(strm_year_meas_mean_fit$fit[[1]])
)




strm_year_meas_mean_drw_sprd$drw[[1]] |> 
  summarise(across(starts_with("b_"), ~sum(. > 0)/6000))
  
hypothesis(strm_year_meas_mean_drw_sprd$drw[[1]], "b_year_scl > 0")

#HC W:D sort of illustrative?
strm_year_meas_mean_drw_sprd$gg[[1]] + strm_year_meas_mean_pp_gg$gg[[1]]
strm_year_meas_mean_fit$fit[[1]] |> posterior::summarise_draws() |> print(n = Inf)
ce <- conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm", prob = 0.05)
ce[[1]] |> filter(str_detect(cmplx_strm, "Stavis"), between(year_scl, -0.02, 0.02))
plot(ce, plot = F)[[1]] + 
  geom_hline(yintercept = exp(2.61)) +
  geom_hline(yintercept = exp(2.61) + exp(0.0796), color = "blue") +
  geom_hline(yintercept = exp(2.56 + 0.0796), color = "pink") +
  geom_hline(yintercept = exp(2.61 + 0.0796), color = "grey") 

conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm", prob = 0.05, re_formula = NULL)[[1]] |> 
  filter(str_detect(cmplx_strm, "Stavis"), between(year_scl, -0.02, 0.02))

conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm", prob = 0.05) #posterior_epred
conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm", prob = 0.05, method = "posterior_predict")
plot(conditional_effects(strm_year_meas_mean_fit$fit[[1]], effects = "year_scl:cmplx_strm", prob = 0.05, method = "posterior_linpred"), plot = F)[[1]] + geom_hline(yintercept = 2.66)

#Stavis: ~14.42 at 0
exp(2.61) #13.599 b_int mean
exp(2.66) #14.296 b_int med
exp(2.56) + exp(0.0796) #14.0186 int + re_int
exp(2.56 + 0.0796) #same as exp(2.56)*exp(0.0796) #14.01 int + re_int
exp(2.61) + exp(0.0796) #14.68 b_int mean + Stavis re_int mean
exp(2.61 + 0.0796) #same as exp(2.61)*exp(0.0796) #14.7

exp(2.56) + exp(2.61)

exp(2.61) + exp(0.0258)*-1.63
exp(2.61) + exp(0.0258)*1.63
exp(2.61) + exp(0.0258*-1.63)
exp(2.61) + exp(0.0258*1.63)

strm_year_meas_mean_pp$pp[[1]] |> filter(str_detect(cmplx_strm, "Stavis")) |> summarise(m = median(.prediction))
...
d |> 
    ...
    name = factor(
      name, 
      levels = c("pct_pools","pool_bnk_depth", "bankfull_W_D","pct_gravel", "vlwd_per100","vlwd_vol"),
      labels = c("Ppn. pool","Pool depth", "Bankfull W:D","Ppn. gravel", "VLWD/100m","VLWD vol.")
      )
  ) |> 
...
ggsave(filename = "rev_fig_site_range_ridges.png", width = 11, height = 9, dpi = 300, bg = "white")
...
...left for spring 2026?

# INITIAL REVISION TESTING: was resto detectable treatment vs reference: whole-stream treat-ref diff series
...
#also have Johansen for 3 or more series: https://www.quantstart.com/articles/Johansen-Test-for-Cointegrating-Time-Series-Analysis-in-R/
#would reduce to 3 cmplx * 6 meas = 18 tests, but not exactly trivial to interpret
#looks possible/likely that rejecting no coint across the full set (r=0) could still have a situation of "some" coint among series
...
#perhaps too few observations is contributing
